# -*- coding: utf-8 -*-
"""
Full SimPad E2E test (steps) using relative UI-map controls and client-only
...
def test_full_simpad_e2e_with_verification(app_ctx, request):
    ...
    rect = get_client_rect(hwnd)  # пригодится для некоторых проверок

    # ---------------------- BASIC NAVIGATION ----------------------
    with step(request, "Open Manual Mode → Standardized Patient → Healthy", hwnd, artifacts):
    ...
    # ---------------------- NAME SESSION --------------------------
    with step(request, "Set Session Name", hwnd, artifacts):
    ...
    # ---------------------- INSTRUCTOR ----------------------------
    with step(request, "Set Instructor", hwnd, artifacts):
    ...
    # ---------------------- PARTICIPANT ---------------------------
    with step(request, "Set Participant #1", hwnd, artifacts):
    ...
    # ---------------------- SESSION OK / START --------------------
    with step(request, "Confirm Session and Start", hwnd, artifacts):
    ...
    # ---------------------- HR SCREEN (ADJUST) --------------------
    with step(request, "Adjust HR: open slider, drag, activate", hwnd, artifacts):
    ...
    # ---------------------- MESSAGE SCREEN ------------------------
    with step(request, "Message screen: open and select 'Coughing'", hwnd, artifacts):
    ...
    # ---------------------- END / QUIT ----------------------------
    with step(request, "End and Quit", hwnd, artifacts):
